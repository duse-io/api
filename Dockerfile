...
RUN cd /tmp && curl ftp://ftp.ruby-lang.org/pub/ruby/2.1/ruby-2.1.4.tar.gz | tar xz
RUN cd /tmp/ruby-2.1.4 && ./configure --disable-install-rdoc && make && sudo make install
RUN rm -rf /tmp/ruby-2.1.4
