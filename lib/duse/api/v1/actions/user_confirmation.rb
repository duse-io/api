...
class User
  class Confirmation
    def resend(params)
      ...
      fail Duse::API::NotFound if user.nil?
      fail Duse::API::AlreadyConfirmed if user.confirmed?
    ...
    def confirm(params)
      ...
      fail Duse::API::NotFound if token.nil?
      fail Duse::API::AlreadyConfirmed if token.user.confirmed?
      token.user.confirm!
      token.destroy
    end
  end
