describe Duse::Config do
  ...
  it 'should return false if no sentry dsn is set' do
    expect(Duse::Config.new.use_sentry?).to be false
  end

  it 'should return true if a sentry dsn is set' do
    expect(Duse::Config.new(sentry_dsn: 'test').use_sentry?).to be true
  end
  ...
  it 'should return https when ssl is true' do
    config = Duse::Config.new(ssl: 'true')
